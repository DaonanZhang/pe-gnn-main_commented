...
# 这个函数 latlon_to_cart 的作用是将经度（lon）和纬度（lat）坐标转换为笛卡尔坐标系中的三维坐标。
...
# 这个函数 knn_to_adj 的作用是将 K 最近邻（K Nearest Neighbors，KNN）图表示转换为邻接矩阵的形式。
# KNN 图通常用于描述数据点之间的近邻关系，其中每个数据点与其 K 个最近的邻居连接。邻接矩阵是一种常见的图表示方法，用于表示图中节点之间的连接关系。
...
# data normalization for inputed pytorch tensor
...
# 总之，这个函数用于计算局部莫兰指数，并根据需要对其进行归一化和处理 NaN 值。局部莫兰指数通常用于空间统计分析，用于衡量地理空间数据的空间自相关性。
